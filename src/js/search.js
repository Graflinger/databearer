...
(function () {
  ...
  async function loadSearchIndex() {
    try {
      ...
      searchResults.innerHTML =
        '<p class="search-info">Fehler beim Laden des Suchindex.</p>';
    }
  }
  ...
  function performSearch() {
    ...
    if (query.length < 2) {
      searchResults.innerHTML =
        '<p class="search-info">Bitte geben Sie mindestens 2 Zeichen ein.</p>';
      return;
    }
    ...
    const results = searchIndex.filter((item) => {
      ...
      const topicMatch = item.topic?.some((t) =>
    ...
    if (results.length === 0) {
      searchResults.innerHTML = `
        <div class="no-results">
          ...
          <p>Ihre Suche nach "<strong>${escapeHtml(
            query
          )}</strong>" ergab keine Treffer.</p>
